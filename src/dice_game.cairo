...
//dice game contract allows players to guess anumber btn 1 and 6 during an active game window.
//the contract owner then has the ability to toggle the game window to disable new guesses from players.
//to determine the winning number, contract owner calls the contract owner calls the request_randomness_from_pragma
//function to request a random number from the pragma VRF oracle.

//==once the random number is received thru the receive_random_words callback func, it is stored in the 
//last_random_number storage variable. each player has to call process_game winners function to determine
//if they have worn or lost.

//the last_random_number generated  is then reduced to anumber btn 1 & 6, and compared to the guesses of the players
//stored in the user_guesses mapping which leads to the emission of an event GameWinner or GameLost.
...
mod DiceGame {
    ...
    impl DiceGame of super::IDiceGame<ContractState> {
        fn guess(ref self: ContratState, guess:u8) {
            assert(self.game.read(), 'GAME_INACTIVE');
    ...
    impl PragmaVRFOracle of super::IPragmaVRF<ContractState> {
        ...
        fn request_randomness_from_pragma(
            ...
            //you would need to send some ETH to this contract first to discover thefees
